...
def run_training(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if 'target_column' not in config['dataset']:
        dataset_name = config['dataset']['name']
        logger.info(f"Target column not specified for {dataset_name}, using default...")
        
        # Set default target column based on dataset name
        if dataset_name == 'iris':
            ...
            config['dataset']['target_column'] = 'class'
        elif dataset_name == 'fetalhealth':
            config['dataset']['target_column'] = 'fetal_health'
            ...
            config['dataset']['target_column'] = 'class'
        elif dataset_name == 'digits_binary_digit0':
            config['dataset']['target_column'] = 'target'
        elif dataset_name == 'customers':
            config['dataset']['target_column'] = 'class'
        elif dataset_name == 'liverdiagnostic':
            config['dataset']['target_column'] = 'class'
        elif dataset_name == 'heart':
            config['dataset']['target_column'] = 'target'
        else:
            config['dataset']['target_column'] = 'target'  # default
        
        logger.info(f"Using target column: {config['dataset']['target_column']}")
...
def structural_prune(config: Dict[str, Any]) -> None:
    ...
    if config['pruning']['enabled']:
        ...
        # Calculate target number of neurons to prune based on target_pruning_rate
        target_pruning_rate = config['pruning']['validation'].get('target_pruning_rate', 0.6)
        target_neurons_to_prune = int(model.hidden.out_features * target_pruning_rate)
        
        # Make sure we have enough candidates or add more based on importance scores
        if len(pruning_candidates) < target_neurons_to_prune:
            # Get all neurons not already in pruning_candidates
            remaining_neurons = [i for i in range(model.hidden.out_features) if i not in pruning_candidates]
            # Sort by importance score (ascending - less important first)
            remaining_neurons.sort(key=lambda x: correlation_results['importance_scores'][x])
            # Add more neurons up to the target
            additional_neurons = remaining_neurons[:target_neurons_to_prune - len(pruning_candidates)]
            pruning_candidates.extend(additional_neurons)
        elif len(pruning_candidates) > target_neurons_to_prune:
            # Sort by importance score and keep only the least important ones
            pruning_candidates.sort(key=lambda x: correlation_results['importance_scores'][x])
            pruning_candidates = pruning_candidates[:target_neurons_to_prune]
        ...
        # Log pruning summary
        logger.info(f"PRUNING SUMMARY: {len(pruning_candidates)} out of {model.hidden.out_features} neurons were pruned ({len(pruning_candidates)/model.hidden.out_features*100:.1f}%)")
        logger.info(f"Original accuracy: {original_accuracy:.2f}%")
        logger.info(f"Pruned accuracy: {pruned_accuracy:.2f}%")
        logger.info(f"Accuracy drop: {(original_accuracy - pruned_accuracy):.2f}%")
